...
def PlainConv(in_ch, out_ch):
    return nn.Sequential(OrderedDict([("conv", Conv3d(in_ch, out_ch, 3, 1, 1, bias=False)), ("bn", BatchNorm3d(out_ch) if UseBN else nn.Identity()), ("attn", TransformerBlock(out_ch, bias=True))]))


def DownConv(in_ch, out_ch):
    return nn.Sequential(
        OrderedDict([("conv", nn.Conv3d(in_ch, out_ch, 3, (1, 2, 2), 1, bias=False)), ("bn", BatchNorm3d(out_ch) if UseBN else nn.Identity()), ("attn", TransformerBlock(out_ch, bias=True))])
    )

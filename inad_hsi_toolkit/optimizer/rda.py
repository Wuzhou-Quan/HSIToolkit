...
class RDA(Optimizer):
    ...
    def shrink(self, i, bar_g, lambda_, gamma):
        ...
        x[mask] = -math.sqrt(i) / gamma * (bar_g[mask] - lambda_ * torch.sign(bar_g[mask]))
        return x
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            for p in group["params"]:
                ...
                mask = abs(state["w_0"] - math.sqrt(i) / gamma * state["bar_g"]) > math.sqrt(i) / gamma * lambda_

                if len(p.shape) > 1:  # weights
                    x[mask] = state["w_0"][mask] - math.sqrt(i) / gamma * (state["bar_g"][mask] + lambda_ * torch.sign(state["w_0"][mask] - math.sqrt(i) / gamma * state["bar_g"][mask]))
                    p.data.copy_(x)
                else:
                    x[mask] = state["w_0"][mask] - math.sqrt(i) / gamma * (state["bar_g"][mask])
                    p.data.copy_(x)
